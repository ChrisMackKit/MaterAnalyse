...
try:
    ...
    caliMale = f"SELECT TVS_Score FROM results WHERE State = 'California' AND Gender = '1'"
    cursorCaliMale.execute(caliMale)
    caliMale_Score = cursorCaliMale.fetchall()
    caliFemale = f"SELECT TVS_Score FROM results WHERE State = 'California' AND Gender = '2'"
    cursorCaliFemale.execute(caliFemale)
    caliFemale_Score = cursorCaliFemale.fetchall()
    caliDem = f"SELECT TVS_Score FROM results WHERE State = 'California' AND Political_Leaning = '1'"
    cursorCaliDem.execute(caliDem)
    caliDem_Score = cursorCaliDem.fetchall()
    caliRep = f"SELECT TVS_Score FROM results WHERE State = 'California' AND Political_Leaning = '2'"
    cursorCaliRep.execute(caliRep)
    caliRep_Score = cursorCaliRep.fetchall()
    caliInd = f"SELECT TVS_Score FROM results WHERE State = 'California' AND Political_Leaning = '3'"
    cursorCaliInd.execute(caliInd)
    caliInd_Score = cursorCaliInd.fetchall()

    geoMale = f"SELECT TVS_Score FROM results WHERE State = 'Georgia' AND Gender = '1'"
    cursorGeorgiaMale.execute(geoMale)
    geoMale_Score = cursorGeorgiaMale.fetchall()
    geoFemale = f"SELECT TVS_Score FROM results WHERE State = 'Georgia' AND Gender = '2'"
    cursorGeorgiaFemale.execute(geoFemale)
    geoFemale_Score = cursorGeorgiaFemale.fetchall()
    geoDem = f"SELECT TVS_Score FROM results WHERE State = 'Georgia' AND Political_Leaning = '1'"
    cursorGeorgiaDem.execute(geoDem)
    geoDem_Score = cursorGeorgiaDem.fetchall()
    geoRep = f"SELECT TVS_Score FROM results WHERE State = 'Georgia' AND Political_Leaning = '2'"
    cursorGeorgiaRep.execute(geoRep)
    geoRep_Score = cursorGeorgiaRep.fetchall()
    geoInd = f"SELECT TVS_Score FROM results WHERE State = 'Georgia' AND Political_Leaning = '3'"
    cursorGeorgiaInd.execute(geoInd)
    geoInd_Score = cursorGeorgiaInd.fetchall()

    ohioMale = f"SELECT TVS_Score FROM results WHERE State = 'Ohio' AND Gender = '1'"
    cursorOhioMale.execute(ohioMale)
    ohioMale_Score = cursorOhioMale.fetchall()
    ohioFemale = f"SELECT TVS_Score FROM results WHERE State = 'Ohio' AND Gender = '2'"
    cursorOhioFemale.execute(ohioFemale)
    ohioFemale_Score = cursorOhioFemale.fetchall()
    ohioDem = f"SELECT TVS_Score FROM results WHERE State = 'Ohio' AND Political_Leaning = '1'"
    cursorOhioDem.execute(ohioDem)
    ohioDem_Score = cursorOhioDem.fetchall()
    ohioRep = f"SELECT TVS_Score FROM results WHERE State = 'Ohio' AND Political_Leaning = '2'"
    cursorOhioRep.execute(ohioRep)
    ohioRep_Score = cursorOhioRep.fetchall()
    ohioInd = f"SELECT TVS_Score FROM results WHERE State = 'Ohio' AND Political_Leaning = '3'"
    cursorOhioInd.execute(ohioInd)
    ohioInd_Score = cursorOhioInd.fetchall()

    nevadaMale = f"SELECT TVS_Score FROM results WHERE State = 'Nevada' AND Gender = '1'"
    cursorNevadaMale.execute(nevadaMale)
    nevadaMale_Score = cursorNevadaMale.fetchall()
    nevadaFemale = f"SELECT TVS_Score FROM results WHERE State = 'Nevada' AND Gender = '2'"
    cursorNevadaFemale.execute(nevadaFemale)
    nevadaFemale_Score = cursorNevadaFemale.fetchall()
    nevadaDem = f"SELECT TVS_Score FROM results WHERE State = 'Nevada' AND Political_Leaning = '1'"
    cursorNevadaDem.execute(nevadaDem)
    nevadaDem_Score = cursorNevadaDem.fetchall()
    nevadaRep = f"SELECT TVS_Score FROM results WHERE State = 'Nevada' AND Political_Leaning = '2'"
    cursorNevadaRep.execute(nevadaRep)
    nevadaRep_Score = cursorNevadaRep.fetchall()
    nevadaInd = f"SELECT TVS_Score FROM results WHERE State = 'Nevada' AND Political_Leaning = '3'"
    cursorNevadaInd.execute(nevadaInd)
    nevadaInd_Score = cursorNevadaInd.fetchall()

    louisianaMale = f"SELECT TVS_Score FROM results WHERE State = 'Louisiana' AND Gender = '1'"
    cursorLouisianaMale.execute(louisianaMale)
    louisianaMale_Score = cursorLouisianaMale.fetchall()
    louisianaFemale = f"SELECT TVS_Score FROM results WHERE State = 'Louisiana' AND Gender = '2'"
    cursorLouisianaFemale.execute(louisianaFemale)
    louisianaFemale_Score = cursorLouisianaFemale.fetchall()
    louisianaDem = f"SELECT TVS_Score FROM results WHERE State = 'Louisiana' AND Political_Leaning = '1'"
    cursorLouisianaDem.execute(louisianaDem)
    louisianaDem_Score = cursorLouisianaDem.fetchall()
    louisianaRep = f"SELECT TVS_Score FROM results WHERE State = 'Louisiana' AND Political_Leaning = '2'"
    cursorLouisianaRep.execute(louisianaRep)
    louisianaRep_Score = cursorLouisianaRep.fetchall()
    louisianaInd = f"SELECT TVS_Score FROM results WHERE State = 'Louisiana' AND Political_Leaning = '3'"
